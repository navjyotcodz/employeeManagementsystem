...
?>
 <div class="wrapper">
   <form id="studentForm" name="frm_stud" method="POST" enctype="multipart/form-data">
	...
	?>
		echo "<option value="<?php echo $id?>" <? if($id == $category){?>selected<?} ?>><?php echo $res["category_name"];?></option>";	 
	<?php
	}
	?>
	</select></div>
	...
	while($res=mysqli_fetch_array($result)){
		$id=$res['id'];
		?>
		echo " <option  value="<?php echo $id?>" <?if(in_array($id,$ind) == $id){?>selected<?} ?>><?php echo $res["industry_name"];?></option>";
		<?php
	}
	?>
	</select></div><br>
